...
The easiest contribution to make is to [file an issue](https://github.com/TutteInstitute/hirank/issues/new).
and do a cursory search of [existing issues](https://github.com/TutteInstitute/hirank/issues?utf8=%E2%9C%93&q=is%3Aissue).
...
[fork the project](https://github.com/TutteInstitute/hirank/issues#fork-destination-box)
...
HiRank uses `pytest`. The tests live under `hirank/tests/`.
...
Run all tests:
```shell
uv run pytest hirank/tests -v
```
Run a specific test file:
```shell
uv run pytest hirank/tests/test_hirank.py -v
```
Run tests with coverage:
```shell
uv run pytest hirank/tests --show-capture=no -v --disable-warnings \
  --junitxml=junit/test-results.xml \
  --cov=hirank/ --cov-report=xml --cov-report=html
```
